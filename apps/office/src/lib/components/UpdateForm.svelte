<script lang="ts">
	...
	function loadFile(event) {
			...
			const outputLiImg = document.createElement('img');
			outputLiImg.src = URL.createObjectURL(event.target.files[0]);
			outputLi.appendChild(outputLiImg);
			output.appendChild(outputLi);
			output.onload = function() {
					URL.revokeObjectURL(outputLiImg.src); // free memory
			};
	}
...
<form id="Buildings">
	...
	<details open={openedDetails === 'sum'}>
		...
		<!--		Hidden inputs       -->
	<div class="hidden">
		<input id="Nummeraanduidingidentificatie"
