<script>
	...
	onMount(() => {
		...
		async function fetchData() {
			try {
				const response = await fetch('constants/bag-database.csv');
	...
	function checkInputs() {
		...
		if (trimmedPostcode && trimmedHnumber) {
			const match = data.find(
				(item) =>
					item.Postcode === trimmedPostcode.toUpperCase() &&
					item.Huisnummer === trimmedHnumber
			);
...
<form id="Buildings">
	<details open={openedDetails === "anno"}>
		...
		<div class="step-content">
			<fieldset form="Buildings" class="ANNO">
				<div>
					<label for="postcode">
						Postcode:
						<input
				...
				<div>
					<label for="street">
						Straat:
						<input id="street" name="Naam" readonly required type="text" />
					</label>
				</div>
				<div>
	...
	<details open={openedDetails === "details"}>
		...
		<div class="step-content">
			<fieldset form="Buildings" class="details">
				<label for="address">
					Adres
				...
				<label for="description">
					Omschrijving
					<textarea cols="50" id="description" name="description" rows="4">
					</textarea>
				</label>
			</fieldset>
		</div>
	...
	<details open={openedDetails === "img"}>
		...
		<div class="step-content">
			<!-- Question 5 -->
			<fieldset form="Buildings" class="img">
				<div>
					<label for="image">
					...
					<LinkButton
						...
						size="large"
						subtle={true}>Bekijk de beeldbank
					</LinkButton
					>
				</div>
				<ul>
